...
/*---                                                              ---*/
/*--- Implementation for Linux, FreeBSD, Darwin and Solaris        ---*/
/*---                                            aspacemgr-linux.c ---*/
/*--------------------------------------------------------------------*/
...
#define MAPFLAGS (VKI_MAP_ANONYMOUS | VKI_MAP_STACK)
#else
#define MAPFLAGS (VKI_MAP_ANONYMOUS)
#endif
   ...
   init_nsegment( &seg );
   seg.kind   = (flags & MAPFLAGS) ? SkAnonC : SkFileC;
   seg.start  = a;
   seg.end    = a + len - 1;
   seg.hasR   = toBool(prot & VKI_PROT_READ);
   seg.hasW   = toBool(prot & VKI_PROT_WRITE);
...
#endif // #elif defined(VGO_solaris)
