...
static void setup_client_dataseg ( SizeT max_size )
{
   ...
   /* XXX We make the data segment (heap) executable because? Why, on FreeBSD?
...
void VG_(ii_finalise_image)( IIFinaliseImageInfo iifii )
{
   ...
   /* On FreeBSD we get client_{ip/sp/toc}, and start the client with
      all other registers zeroed. XXX is this correct? - copied from Linux */
