...
{
   *flags |= SfMayBlock;
   PRINT("sys_posix_fallocate ( %ld, 0x%lx, %lu )", ARG1, ARG2, ARG3);
...
{
   PRINT("sys_posix_fadvise ( %ld, 0x%lx, %lu, %ld )", ARG1, ARG2, ARG3, ARG4);
   PRE_REG_READ4(long, "posix_fadvise",
                 unsigned int, fd, unsigned long, offset, unsigned long, len,
                 int, advice);
}
...
// BSDXY(__NR_msgctl7,			sys_msgctl7),			// 224
